...
export class SlackPuppet {
  ...
  async openChannel(channel: string): Promise<SlackChannelPuppet> {
    ...
    const searchBox = await this.page.waitForSelector(
      ".c-search__input_box__input > *[contenteditable=true]"
    )
...
const forAllMessages = async (
  ...
  while (true) {
    ...
    let visibleMessages = await Promise.all(
      (await container.$$(".c-message_kit__message")).map(async (e) => [
        e,
        await page.evaluate(
          (e) =>
            e
    ...
    let newMessages = visibleMessages.filter(
      ([e, id]) => !lastMessageIds.includes(id)
    )
    ...
    for (const [elemMessage, id] of newMessages) {
      await fn(elemMessage, id, {
        open: async (fn: MaybeThreadFunc) => {
          const elemReplyBar = await elemMessage.$(
            ".c-message_kit__thread_replies .c-link"
          )
          if (elemReplyBar) {
            ...
            await page.waitForSelector(
              ".p-workspace__secondary_view .c-message_kit__message"
            )
            await fn(new SlackThreadPuppet(page))
            const elemClose = await page.$(
              ".p-workspace__secondary_view *[data-qa=close_flexpane]"
            )
    ...
    await page.evaluate(
      (cont, y) => cont.querySelector(".c-scrollbar__hider").scrollBy(0, y),
      container,
      height
    )
...
export class SlackChannelPuppet {
  ...
  async scrollToOldest() {
    while ((await this.page.$(".p-global_banner")) === null) {
      await this.page.evaluate(() =>
        document
          .querySelector(".c-message_list > .c-scrollbar__hider")!
          .scrollBy(0, -1e5)
      )
      await this.page.waitFor(500) // FIXME remove all timers
    }
  ...
  async forAllMessages(fn: MessageFunc): Promise<void> {
    return await forAllMessages(
      this.page,
      (await this.page.$(".p-workspace__primary_view"))!,
      fn
    )
  }
}
...
export class SlackThreadPuppet {
  ...
  async forAllMessages(fn: MessageFunc): Promise<void> {
    return await forAllMessages(
      this.page,
      (await this.page.$(".p-workspace__secondary_view"))!,
      fn
    )
  }
}
