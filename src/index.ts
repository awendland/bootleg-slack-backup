...
if (process.env.LOG_LEVEL) Log.setLevelByString(process.env.LOG_LEVEL)
...
export const run = async () => {
  ...
  try {
    ...
    if (process.env.DEBUG) await installMouseHelper(page)
    ...
    for (const channel of CHANNELS!.split(",")) {
      ...
      await channelPuppet.forAllMessages(
        async (msgElem, msgId, maybeThread) => {
          ...
          try {
            ...
            await maybeThread.open(async (threadPuppet) => {
              ...
              await threadPuppet.forAllMessages(
                async (threadMsgElem, threadMsgId) => {
                  Log.trace(`--- sub message [${threadMsgId}]`)
                  if (threadMsgId !== msgId) {
                    message.thread.push(
                      await extractMsgInfo(page, threadMsgElem)
                    )
                  }
                }
              )
